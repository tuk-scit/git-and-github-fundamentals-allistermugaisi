...
const Researchers = () => {
	...
	let researchers = useSelector((state) => state.researchers?.researchers);
	...
	const [deletedResearcher, setDeletedResearcher] = useState([]);
	...
	const onSubmit = async (data, e) => {
		...
		await dispatch(createResearcher(data));
		await dispatch(getResearchers());
	...
	const onSubmitEdit = async (data, e) => {
		...
		await dispatch(updateResearcher(payload));
		await dispatch(getResearchers());
	...
	const handleDeleteResearcher = (client) => {
		setDeletedResearcher(client);
		setOpenDeletePopup(true);
	};

	const confirmDeleteResearcher = async () => {
		setButtonLoading(true);
		await dispatch(deleteResearcher(deletedResearcher));
		await dispatch(getResearchers());
		setButtonLoading(false);
		...
		<>
					...
					<div className="w-full overflow-x-auto no-scrollbar">
						<table className="w-full whitespace-nowrap">
							<thead className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:text-gray-400 dark:bg-gray-800">
								<tr>
									<td className="px-4 py-3">Researcher Name</td>
												...
												<td className="px-4 py-3">
													<div className="flex items-center justify-end text-right">
														<div className="p-2 cursor-pointer text-gray-400 hover:text-green-600">
															<IconButton
																onClick={(e) => handleEditPopup(researcher, e)}
															>
																<svg
																	stroke="currentColor"
																	fill="none"
														...
														<div className="p-2 cursor-pointer text-gray-400 hover:text-red-600">
															<IconButton
																onClick={(e) =>
																	handleDeleteResearcher(researcher, e)
																}
															>
																<svg
																	stroke="currentColor"
			...
			>
				...
				<DialogActions>
					...
					<CustomButton
						onClick={confirmDeleteResearcher}
						disabled={buttonLoading ? true : false}
						loading={buttonLoading}
						variant="contained"
					>
